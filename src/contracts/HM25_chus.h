...
private:
    ...
    PUBLIC_PROCEDURE(Deposit)
        ...
        auto& userBalance = getUserBalance(qpi.invocator()); //ERROR ?
        userBalance.balance += input.amount; //ERROR inicializar userBalance
...
public:
    ...
    INITIALIZE
        admin = NULL_ID; // VULNERABILITY 10: Admin not properly initialized   // ERROR
        totalDeposits = 0;  // ERROR
        reentrancyLock = false;  // ERROR
    _
};
